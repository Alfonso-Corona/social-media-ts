...
const createComment = async (
  ...
  const {error} = await supabase.from("comments").insert({
    ...
    parent_comment_id: newComment.parent_content_id || null,
    user_id: userId,
    author: author
  })
...
export const CommentSection = ({ postId }: Props) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    mutate({content: newCommentText, parent_content_id: null});
    setNewCommentText("");
  };
  ...
  return (
    <div className="mt-6">
      ...
      {user ? (
        ...
        <p className="text-gray-600 mb-4">You mus be logged in to post a comment... :c</p>
      )}
